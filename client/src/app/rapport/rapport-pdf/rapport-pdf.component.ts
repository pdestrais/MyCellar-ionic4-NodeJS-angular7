	...
	public cellarToPDF() {
		...
		this.pouch.getDocsOfType('vin').then((vins) => {
			...
			this.typesGrouping = d3
				...
				.entries(
					this.vins.filter(function(d) {
						return d.nbreBouteillesReste != 0;
					})
				);
			...
			this.typesGrouping.forEach((itemType: any, iType: any) => {
				...
				this.typesOrigineGrouping = d3
					...
					.entries(
						this.vins.filter(function(d) {
							return d.nbreBouteillesReste != 0 && d.type.nom == itemType.key;
						})
					);
				this.typesOrigineGrouping.forEach((itemOrigine: any, iOrigine: any) => {
					...
					this.typesOrigineYearGrouping = d3
						.nest()
						.key(function(d: any) {
							return d.annee;
						})
					...
					this.typesOrigineYearGrouping.forEach((itemYear: any, iYear: any) => {
						...
						this.vinsFiltered.forEach((wineItem: any, iWineItem: number) => {
							...
							y = y + 0.3;
							if (iWineItem == 0) this.doc.text(1.2, y, wineItem.annee + ' (' + itemYear.value + ')');
